...
class getRecord {
  ...
  disconnect() {}
  update(config) {
    if (this.entityName !== config.entityName || this.entityId !== config.entityId) {
    ...
    if (this.entityName && this.entityId) {
      try {
        ...
        if (individualRecordCache.has(cacheKey)) {
          ...
          let result = await fetch(new Request(getRecordApiUrl(this.entityName, this.entityId)));
          if (result.ok) {
      ...
      try {
        if (!recordCache.has(this.entityName)) {
          if (!refreshesInProgress.has(this.entityName)) {
            refreshesInProgress.set(this.entityName, getRecordsFromApi(this.entityName));
          }
...
_registerDecorators2(getRecords, {
  fields: ["entityName"]
});
...
function updateRecord(entityName, id, data) {
  let endpoint = getApiEndpoint();
  return performDml(`${endpoint}/${entityName}/${id}`, 'PATCH', entityName, id, data);
}
export { createRecord, updateRecord, getRecord, getRecords, getRecordsFromApi, getRecordApiUrl, getRecordUiUrl };