...
function createComponentDef(Ctor) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    if (!isUndefined$1(ctorShadowSupportMode)) {
      assert.invariant(ctorShadowSupportMode === "any" /* Any */ || ctorShadowSupportMode === "reset" /* Default */, `Invalid value for static property shadowSupportMode: '${ctorShadowSupportMode}'`);
    }
...
function fallbackElmHook(elm, vnode, renderer) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    const isPortal = !isUndefined$1(context) && !isUndefined$1(context.lwc) && context.lwc.dom === "manual" /* Manual */;
...
function evaluateStylesheetsContent(stylesheets, stylesheetToken, vm) {
  ...
  for (let i = 0; i < stylesheets.length; i++) {
    ...
    if (isArray$1(stylesheet)) {
      ...
      const scopeToken = isScopedCss || vm.shadowMode === 1 /* Synthetic */ && vm.renderMode === 1 /* Shadow */ ? stylesheetToken : undefined;
      // Use the actual `:host` selector if we're rendering global CSS for light DOM, or if we're rendering
      // native shadow DOM. Synthetic shadow DOM never uses `:host`.
      const useActualHostSelector = vm.renderMode === 0 /* Light */ ? !isScopedCss : vm.shadowMode === 0 /* Native */;
...
function hydrateElement(elm, vnode, renderer) {
  if (!hasCorrectNodeType(vnode, elm, 1 /* ELEMENT */, renderer) || !isMatchingElement(vnode, elm, renderer)) {
    return handleMismatch(elm, vnode, renderer);
  }
  vnode.elm = elm;
  const {
